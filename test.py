...
class MyGUI(QMainWindow):
    ...
    def remove_comments(self, text):
        # Check if there is only one '%' character in the text
        if self.count_percent(text) == 1:
            return text  # If there is only one '%', return the original text without any changes.
        ...
        for char in text:
            if char == '%' and not in_comment:
                ...
                result += ' '  # Replace the comment text with a space.
            elif not in_comment:
                result += char  # If we are not in a comment, add the character to the result.
